// Program to verify STM32F onboard ADC
// Nazim Wali
// Embedded Systems Hands-On Exercise #1: ADC Verification
// DUE: 2/22/21
...
int main (void) {
    ...
    while (1) {
        ...
        result = ADC1->DR;              /* read conversion result */
				...
				for(int i = 0; i < 4; i++){						//For loop to output char array contents to LCD
					LCD_data(conv[i]);
					//delayMs(100);
				}
				
				degr = (double)result/4095*180;				//Equation for Degrees
				...
				for(int i = 0; i < 5; i++){						//Print contents of char array
					LCD_data(conv2[i]);
					//delayMs(100);
				}
